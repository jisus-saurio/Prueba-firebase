...
export default function EditUserScreen({ route, navigation }) {
  ...
  useEffect(() => {
    // Verificar si hay cambios - SOLO si originalData ya está cargado
    if (Object.keys(originalData).length > 0) {
      ...
      console.log('Detectando cambios:', {
        nameChanged: formData.name !== originalData.name,
        degreeChanged: formData.universityDegree !== originalData.universityDegree,
        yearChanged: formData.graduationYear !== originalData.graduationYear,
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      const updateData = {
        ...
        graduationYear: parseInt(formData.graduationYear.trim()),
        updatedAt: new Date().toISOString(),
        updatedBy: auth.currentUser?.uid || 'system',
      };

      console.log('Datos a enviar a Firebase:', updateData);
      
      await updateDoc(doc(db, 'users', userId), updateData);
      
      console.log('Usuario actualizado exitosamente');

      // Actualizar originalData para reflejar los cambios
      setOriginalData({
        ...formData,
        graduationYear: String(updateData.graduationYear)
      });
